...
void WorkerCheck()
{
    Worker tmp = Worker("Èãíàò", "Øàðãîðîäñêèé", "Ñåðãååâè÷", 13, 5, 2003, "615gfh5654", "g4hfg5h6f51", "0", "-----", "Áîã", 10024, 13, 5, 2003);

    printf("Worker check ");

    if (tmp.FIO() == "Øàðãîðîäñêèé Èãíàò Ñåðãååâè÷" && tmp.FreeWeeks())
        printf("complited\n");
    else
        printf("failed\n");
}
